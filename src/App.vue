  ...
  created() {
    ...
    Graph.registerNode("dice-er-box", {
        ...
        if (afterList) {
          afterList.forEach((e, i) => {
            ...
            listContainer.addShape("rect", {
              ...
              name: `item-${Math.floor(startIndex) + i}-content_rect`,
              draggable: true,
            });
            //图片
      ...
      update(cfg, item) {
        ...
        const titleShape = group.findAll((ele) => {
          return (ele.get("name") ? ele.get("name") : '').includes('content_rect');
        });
        console.log(titleShape);
        titleShape.forEach((item, i) => {
          const isSelected =
            Math.floor(startIndex) + i === Number(selectedIndex);
          item.attr("stroke", isSelected ? "#fea501" : "#fff");
        });
      },
      getAnchorPoints() {
        return [
